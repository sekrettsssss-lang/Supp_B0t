...
def start(message):
    ...
    # ВАЖНОЕ ИЗМЕНЕНИЕ: Показываем кнопки ТОЛЬКО в личных чатах
    if message.chat.type == "private":
        ...
        # В группе отправляем сообщение БЕЗ КНОПОК и удаляем существующую клавиатуру
        remove_markup = types.ReplyKeyboardRemove()
        bot.send_message(message.chat.id, "Добро пожаловать! Напишите мне в личные сообщения для доступа к функциям бота.", reply_markup=remove_markup)

# Админ меню - ТОЛЬКО в личных чатах
@bot.message_handler(func=lambda m: m.text == '👑 Админ меню' and m.from_user.id in ADMIN_IDS and m.chat.type == "private")
...
# Обработка админ-команд - ТОЛЬКО в личных чатах
@bot.message_handler(func=lambda m: m.from_user.id in ADMIN_IDS and m.text in ['📊 Статистика', '👥 Список пользователей', '📋 Список банов'] and m.chat.type == "private")
...
# Кнопка Забанить пользователя - ТОЛЬКО в личных чатах
@bot.message_handler(func=lambda m: m.from_user.id in ADMIN_IDS and m.text == '🔨 Забанить пользователя' and m.chat.type == "private")
...
# Кнопка Разбанить пользователя - ТОЛЬКО в личных чатах
@bot.message_handler(func=lambda m: m.from_user.id in ADMIN_IDS and m.text == '🔓 Разбанить пользователя' and m.chat.type == "private")
...
# Рассылка сообщений - ТОЛЬКО в личных чатах
@bot.message_handler(func=lambda m: m.from_user.id in ADMIN_IDS and m.text == '📢 Сделать рассылку' and m.chat.type == "private")
...
# Кнопка Назад - ТОЛЬКО в личных чатах
@bot.message_handler(func=lambda m: m.text == '🔙 Назад' and m.chat.type == "private")
def back_to_main(message):
    start(message)

# Основные кнопки меню - ТОЛЬКО в личных чатах
@bot.message_handler(func=lambda m: m.text in ['🆘 Мне нужна поддержка', '💬 Я хочу поговорить'] and m.chat.type == "private")
...
@bot.message_handler(func=lambda m: m.text == '📢 Наш канал' and m.chat.type == "private")
def channel(message):
    user_id = message.from_user.id
    if is_banned(user_id):
        return
...
@bot.message_handler(func=lambda m: m.text == '⭐ Наши отзывы' and m.chat.type == "private")
def reviews(message):
    user_id = message.from_user.id
    if is_banned(user_id):
        return
...
# ВАЖНО: Обработчик для ВСЕХ сообщений в группах - удаляем клавиатуру
@bot.message_handler(func=lambda m: m.chat.type in ["group", "supergroup"])
def handle_group_messages(message):
    # Удаляем ReplyKeyboard в группах
    remove_markup = types.ReplyKeyboardRemove()
    bot.send_message(message.chat.id, "Напишите мне в личные сообщения для доступа к функциям бота.", reply_markup=remove_markup)
...
def handle_all_messages(message):
    ...
    # Пропускаем сообщения из групп (они обрабатываются выше)
    ...
    # СПИСОК ВСЕХ КНОПОК МЕНЮ КОТОРЫЕ НЕ ПЕРЕСЫЛАЕМ В ГРУППУ
    menu_buttons = [
    ...
    # Если это команда - НЕ пересылаем в группу
    ...
    # Если сообщение от пользователя - пересылаем в поддержку (ТОЛЬКО РЕАЛЬНЫЕ СООБЩЕНИЯ)
        ...
        chat_type = "💬 Сообщение"
        if "🆘" in message.text or "поддержк" in message.text.lower() or "помощ" in message.text.lower():
            chat_type = "🆘 Запрос поддержки"
        elif "💬" in message.text or "поболтать" in message.text.lower() or "общени" in message.text.lower():
            chat_type = "💬 Запрос общения"
