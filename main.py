...
def admin_menu(message):
    ...
    btn7 = types.KeyboardButton('🔄 Управление ботом')
...
@bot.message_handler(func=lambda m: m.text == '🔄 Управление ботом' and m.from_user.id in ADMIN_IDS)
def bot_management(message):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    btn1 = types.KeyboardButton('🔄 Перезагрузить бота')
    btn2 = types.KeyboardButton('📟 Статус бота')
    btn3 = types.KeyboardButton('🚪 Мягкий перезапуск')
    btn4 = types.KeyboardButton('🔙 Назад в админку')
    markup.add(btn1, btn2, btn3, btn4)
    
    bot.send_message(message.chat.id, "🔄 Управление ботом:", reply_markup=markup)

# Команды управления ботом
@bot.message_handler(func=lambda m: m.from_user.id in ADMIN_IDS and m.text in ['🔄 Перезагрузить бота', '📟 Статус бота', '🚪 Мягкий перезапуск'])
def handle_bot_commands(message):
    if not check_cooldown(message.from_user.id):
        bot.send_message(message.chat.id, "⏳ Подождите 3 секунды перед следующим действием")
        return
        
    if message.text == '📟 Статус бота':
        stats = get_daily_stats()
        status_text = f"""
📟 **СТАТУС БОТА**

🟢 Бот активен
📊 Пользователей сегодня: {stats['daily_users']}
💬 Сообщений сегодня: {stats['daily_messages']}
👥 Всего пользователей: {stats['total_users']}

⏰ Время сервера: {datetime.datetime.now().strftime("%H:%M:%S")}
...
"""
        ...
        btn_confirm = types.InlineKeyboardButton('✅ Подтвердить', callback_data='restart_confirm')
        btn_cancel = types.InlineKeyboardButton('❌ Отмена', callback_data='restart_cancel')
        markup.add(btn_confirm, btn_cancel)
        
        bot.send_message(
            message.chat.id,
            "⚠️ **ПЕРЕЗАГРУЗКА БОТА**\n\n"
            "Бот будет полностью перезагружен. Это займет 5-10 секунд.\n"
    ...
    elif message.text == '🚪 Мягкий перезапуск':
        bot.send_message(message.chat.id, "🔄 Выполняю мягкий перезапуск...")
        time.sleep(1)
        bot.stop_polling()
        time.sleep(2)
        start_bot()
...
@bot.message_handler(func=lambda m: m.text == '🔙 Назад в админку')
...
@bot.message_handler(func=lambda m: m.chat.type == 'private' and m.text and m.text not in ['🆘 Мне нужна поддержка', '💬 Я хочу поговорить', '📢 Наш канал', '⭐ Наши отзывы', '👑 Админ меню', '📊 Статистика', '📢 Сделать рассылку', '👥 Список пользователей', '🔨 Забанить пользователя', '🔓 Разбанить пользователя', '📋 Список банов', '🔙 Назад', '🔄 Управление ботом', '🔄 Перезагрузить бота', '📟 Статус бота', '🚪 Мягкий перезапуск', '🔙 Назад в админку'])
